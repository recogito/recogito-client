...
import { removeDocumentsFromProject, updateDocumentsSort } from '@backend/helpers';
...
export const DocumentsView = (props: DocumentsViewProps) => {
  ...
  const onDragEnd = useCallback((event) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = props.documents.findIndex((document) => document.id === active.id);
      const newIndex = props.documents.findIndex((document) => document.id === over!.id);
      ...
      updateDocumentsSort(supabase, props.project.id, newDocumentIds)
        .then(({ error }) => {
          if (error) {
            console.log(error);
          }
  ...
  const activeDocument = useMemo(() => props.documents?.find((document) => document.id === activeId), [activeId]);
